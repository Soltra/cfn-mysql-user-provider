...
class MySQLUser(ResourceProvider):
    ...
    def grant_ownership(self):
        ...
        cursor = self.connection.cursor()
        try:
            cursor.execute("GRANT SELECT, INSERT, UPDATE, DELETE, CREATE, DROP, RELOAD, PROCESS, REFERENCES, INDEX, ALTER, SHOW DATABASES, CREATE TEMPORARY TABLES, LOCK TABLES, EXECUTE, REPLICATION SLAVE, REPLICATION CLIENT, CREATE VIEW, SHOW VIEW, CREATE ROUTINE, ALTER ROUTINE, CREATE USER, EVENT, TRIGGER ON *.* TO '%s'@'%s' WITH GRANT OPTION" %
                           (self.mysql_user, '%'))
        finally:
            cursor.close()
